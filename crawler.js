...
async function main() {
  try {
    ...
    const TT_LIST = await playerSearch([207], 0); // playerSearch(시즌넘버, 최소오버롤)
    let TT_RESULTS = await playerPriceValue(TT_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(TT_RESULTS);
